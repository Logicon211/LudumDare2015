...
public class NuclearMan : MonoBehaviour {
		...
		if(Health > 0) {
			...
			if (Input.GetButtonDown ("Laser")) {
				if(Heat >= 1)	{
					...
					laserInstance.GetComponent<Laser>().player = this;
					ChangeHeat (1);
					if (!facingRight) {
				...
				ChangeHeat (10); //lose a bunch of energy after firing to prevent it being spammed
				anim.SetBool("Lasering", false);
				isLasering = false;
				Destroy (laserInstance);
			} else {
	...
	void OnTriggerEnter2D(Collider2D col) {
		if(col.CompareTag("OneWayPlatform")) {
			...
			if(RB.velocity.y < 0 && !(Input.GetAxis("Vertical") < 0) &&  Physics2D.OverlapCircle(groundCheck.position, 0.7f, 1 << LayerMask.NameToLayer("OneWayPlatform"))) {// && transform.position.y - col.transform.position.y > maxDistance) {
				RB.gravityScale = 0;
